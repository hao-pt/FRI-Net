...
def main(args):
    ...
    save_folder = args.save_folder # f'./results'
...
def tensor_to_cv(img_tensor):
    # img_tensor: shape (C, H, W), values in [0, 1] or [0, 255]
...
def evaluate(model, data_loader, save_folder, args, save_primitive=True):
    ...
    json_folder = f"{save_folder}/json"
    ...
    for batched_inputs in metric_logger.log_every(data_loader, 10, header):
        ...
        for b_i in range(bs):
            if not os.path.exists(f"{args.img_folder}/{img_names[b_i]}.png"):
                image_path = f"{args.img_folder}/{args.image_set}/{img_names[b_i]}.png"
            else:
            ...
            for room in room_polys:
                for _ in range(room.shape[0]):
                    cv2.circle(img, room[_].astype(np.uint8), 3, [255, 0, 255], -1)
                    cv2.circle(img, room[(_+1)%room.shape[0]].astype(np.uint8), 3, [255, 0, 255], -1)
                    cv2.line(img, room[_].astype(np.uint8), room[(_+1)%room.shape[0]].astype(np.uint8), [255, 0, 255], 2)
            save_img_path = f'{vis_folder}/{img_name}.png'
            save_path = f'{npy_folder}/{img_name}.npy'

            output_json = [{'image_id': img_name, 
                        'segmentation': room_polys_list[instance_id],
                        'category_id': -1,
                        'id': instance_id,
                        } for instance_id in range(len(room_polys_list))]
            print(f'generate: {img_name}')
            cv2.imwrite(save_img_path, img)
            np.save(save_path, room_polys)
            with open(f'{json_folder}/{img_name}.json', 'w') as json_file:
